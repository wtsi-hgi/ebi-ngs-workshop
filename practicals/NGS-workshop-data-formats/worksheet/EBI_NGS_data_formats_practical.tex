...
\framesubtitle{SAM/BAM Format}
\begin{itemize}
\item You can also use `\texttt{samtools view}' to view or extract regions of a BAM file. (Tip: press `q' to close `less'/`zless').
\end{itemize}
\begin{tcblisting}{listing only, listing options={style=custombash}}
...
\item Generate vcfstats plots
         ...
         \end{tcolorbox}
\item View vcfstats/summary.pdf (e.g. using `\texttt{xdg-open vcfstats/summary.pdf}')
\item Looking at the plots, answer the following questions:
	\begin{itemize}
