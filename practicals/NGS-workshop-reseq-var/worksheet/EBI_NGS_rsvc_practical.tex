...
\item \uncover<2->{Align the lane fastq files and generate paired-end alignment with bwa}
\only<2>{
         \begin{tcolorbox}[fontupper=\scriptsize]
         Change to the \path !Exercise-align/60A-Sc-DBVPG6044/lane1! directory and use `\texttt{bwa mem}' to align and generate a pair-end aligned SAM file. Create the file \path !lane1.sam! with read group ID `\texttt{lane1}' referring to sample `\texttt{60A-Sc-DBVPG6044}', using the option to set the read group header line to `\texttt{$@$RG$\backslash$tID:lane1$\backslash$tSM:60A-Sc-DBVPG6044}'.
         \end{tcolorbox}
}
